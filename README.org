...
Drag a file from your file manager (or =dired=) onto a shell or viewport buffer to attach it to the prompt, the same way a pasted image is. Images get a preview; any other file is attached as an =@path= link. Set =agent-shell-dnd-enabled= to nil to keep Emacs's default drop handling, which opens the file.
