...
Install Qoder CLI following the [[https://docs.qoder.com/cli/installation][Qoder installation instructions]],
then log in once:
...
Qoder CLI includes an ACP server.  Start it from =agent-shell= with
=M-x agent-shell-qoder-start-agent=; the backend launches:

#+begin_src bash
qoder --acp
#+end_src

Qoder uses the CLI's existing login state, so a Qoder subscription can be used
without configuring a separate model API key.  For non-interactive
authentication, set =QODER_PERSONAL_ACCESS_TOKEN=...= in
=agent-shell-qoder-environment=.  See the [[https://docs.qoder.com/cli/acp][Qoder ACP documentation]].
...
- =M-x agent-shell-qoder-start-agent= - Start a Qoder CLI agent session
...
| agent-shell-qoder-acp-command                 | Command and parameters for the Qoder CLI ACP server.                            |
| agent-shell-qoder-environment                 | Environment variables for the Qoder CLI ACP server.                             |
...
| agent-shell-show-cost-indicator               | Whether to show the session's cumulative cost in the header.                    |
...
|                                                       | agent-shell-qoder-start-agent                           | Start an interactive Qoder agent shell.                                       |
