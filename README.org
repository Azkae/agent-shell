...
Fold indicators, fragment labels and links carry their own keymaps, which
take precedence over =agent-shell-mode-map=. Both bind =RET= and =mouse-1=
to =agent-shell-ui-act= by default:

- =agent-shell-ui-fragment-map= - expands/collapses a fragment.
- =agent-shell-ui-action-map= - opens a file link, region link or URL.

They're separate so expanding can be bound apart from opening. For example,
to expand with =TAB= while leaving =RET= to open links:
...
Note this shadows =TAB= navigation (=agent-shell-next-item=) while point is
on a fold indicator or fragment label.
