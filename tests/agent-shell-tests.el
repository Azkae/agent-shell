...
(defconst agent-shell-dnd-test--no-temp-dir "/nonexistent/agent-shell-dnd-tmp/"
  "A `temporary-file-directory' nothing lives under.
...
shell is idle, and `temporary-file-directory' points nowhere so test
files are attached in place."
  (declare (indent 1))
...
(ert-deftest agent-shell--dnd-handle-file-url-busy-shell-queues-test ()
  "A drop on a shell mid-turn is queued, like `agent-shell-send-region'."
...
(ert-deftest agent-shell--dnd-keep-file-test ()
  "Files under `temporary-file-directory' are copied with their extension;
files elsewhere, and files of a project that lives there, stay put."
  (let ((file (make-temp-file "agent-shell-dnd" nil ".png" "pixels"))
        (screenshots-dir (make-temp-file "agent-shell-dnd-shots" t)))
    (unwind-protect
...
(ert-deftest agent-shell--enable-dnd-installs-buffer-local-handler-test ()
  "Enabling drops installs the file handler in this buffer only, ahead of
Emacs's, for the local file: forms and not the host form."
  (let ((global-alist (default-value 'dnd-protocol-alist)))
    (with-temp-buffer
      ...
      (should (eq (cdr (seq-find (lambda (entry)
                  ...
                  'dnd-open-file)))
    (with-temp-buffer
      (let ((agent-shell-dnd-enabled nil))
        (agent-shell--enable-dnd))
      (should-not (local-variable-p 'dnd-protocol-alist)))
