...
(defcustom agent-shell-dnd-enabled t
  "Non-nil attaches files dropped on shell and viewport buffers to the prompt.
When nil, a drop gets Emacs's default handling and opens the file."
  :type 'boolean
  :group 'agent-shell)
...
selection.  A drop on a shell that is mid-turn is queued, the way
`agent-shell-send-region' queues its region.
...
Does nothing when `agent-shell-dnd-enabled' is nil."
  (when agent-shell-dnd-enabled
